...
const CustomerDetails = () => {
  ...
  useEffect(() => {
    ...
    const loadCustomer = async () => {
      try {
        ...
        await loadFollowUps(response.customer.id);
      } catch (error) {
  ...
  const loadFollowUps = async (customerId: number) => {
    try {
      ...
      const response = await getCustomerFollowUps(customerId);

      setFollowUps(response.followUps);
    } catch (error) {
      console.error("Failed to load follow-ups:", error);

      setFollowUpError("Failed to load follow-up history.");
    } finally {
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <section className="rounded-xl border bg-white">
        ...
        <div className="grid gap-5 p-5 sm:grid-cols-2">
          ...
          <InfoItem label="Customer Type" value={customer.customerType} />
        </div>
      </section>
      ...
      <section className="rounded-xl border bg-white">
        ...
        <div className="grid gap-5 p-5 sm:grid-cols-2">
          <InfoItem
            label="Business Name"
            value={customer.businessName || "Not provided"}
          />
